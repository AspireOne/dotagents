...
describe("writeMcpConfigs", () => {
  ...
  it("keeps non-bearer mixed Codex header refs literal", async () => {
    await writeMcpConfigs(["codex"], [{
      ...HTTP_SERVER_WITH_ENV_REFS,
      headers: {
        ...HTTP_SERVER_WITH_ENV_REFS.headers,
        "X-Trace": "prefix ${TRACE_ID}",
      },
    }], projectMcpResolver(dir));
    ...
    expect(childObject(content, "mcp_servers")["authed-api"]).toEqual({
      ...
      env_http_headers: { "X-Api-Key": "API_KEY" },
      bearer_token_env_var: "TOKEN",
      http_headers: { "X-Trace": "prefix ${TRACE_ID}" },
    });
  });
